...
export function useChat(options?: UseChatOptions) {
  ...
  const updateMessage = useCallback((id: string, patch: Partial<Pick<ChatMessage, "role" | "text">>): ChatMessage[] => {
    const next = messagesRef.current.map((message) => (message.id === id ? { ...message, ...patch } : message));
    messagesRef.current = next;
    setMessages(next);
    return next;
  }, []);
